...
from src.models import User, UserSettings, LoginRequest, Token, UserOut, UserSettingsUpdate
from src.crypto import verify_password, create_access_token, decode_access_token
...
async def get_me(current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    result = await session.exec(select(UserSettings).where(UserSettings.user_id == current_user.id))
